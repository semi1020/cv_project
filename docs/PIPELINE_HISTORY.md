...
| **clip_main_siglip2_L_512** | 2026-05-11 | **SigLIP2-L-512** zero-shot canonical | **89.2%** (3502) | **0.0%** | **89.2%** | ~29분 | +2.6pp vs SigLIP-L, 더 빠름. Stage A 신규 baseline |
| **e2e_v2_siglip2_L512_no_b** | 2026-05-11 | **SigLIP2-A canon + B skip + SigLIP2-C** | main 89.2% / sub-given-main **77.2%** / **E2E 68.9%** | 0% | - | ~30분 | **신규 정식 baseline. +19.5pp E2E vs CLIP-L** |

데이터 원본: [experiments/summary.csv](../experiments/summary.csv), [clip_main_canonical_eval.txt](../experiments/clip_main_canonical_eval.txt), [clip_main_multi_eval.txt](../experiments/clip_main_multi_eval.txt), [e2e_v2_no_b_eval.txt](../experiments/e2e_v2_no_b_eval.txt), [e2e_v2_full_eval.txt](../experiments/e2e_v2_full_eval.txt), [clip_main_canonical_siglip-large-patch16-384_eval.txt](../experiments/clip_main_canonical_siglip-large-patch16-384_eval.txt), [clip_main_canonical_metaclip-l14-fullcc2.5b_eval.txt](../experiments/clip_main_canonical_metaclip-l14-fullcc2.5b_eval.txt), [e2e_v2_siglip-large-patch16-384_canonical_no_b_eval.txt](../experiments/e2e_v2_siglip-large-patch16-384_canonical_no_b_eval.txt), [clip_main_canonical_siglip2-large-patch16-512_eval.txt](../experiments/clip_main_canonical_siglip2-large-patch16-512_eval.txt), [e2e_v2_siglip2-large-patch16-512_canonical_no_b_eval.txt](../experiments/e2e_v2_siglip2-large-patch16-512_canonical_no_b_eval.txt)
...
### 2.9 Pipeline v2 SigLIP2-L-512 E2E — 정식 baseline 확립 (2026-05-11)

**실험:** [11_e2e_pipeline_v2.py](../11_e2e_pipeline_v2.py) `--model-id google/siglip2-large-patch16-512 --skip-stage-b`.

**결과:**

| 모델 | main | sub-given-main | E2E |
|---|---|---|---|
| CLIP-L (Pipeline v2 baseline) | 79.5% | 62.1% | 49.4% |
| SigLIP-L | 86.6% | 74.6% | 64.6% |
| **SigLIP2-L-512** | **89.2%** (3502) | **77.2%** (2705/3502) | **68.9%** (2705/3927) |

**vs CLIP-L:** +9.7pp main / +15.1pp sub-given-main / **+19.5pp E2E (+766건)** — 모델 swap만으로.

**Sub-acc 클래스별 변화 (SigLIP-L → SigLIP2-L-512):**

큰 회복 — SigLIP-L의 sub 회귀가 자동 해소:
| 클래스 | SigLIP-L sub | SigLIP2-L-512 sub | Δ | 비고 |
|---|---|---|---|---|
| 거울 | 50.0% | **79.3%** | +29.3pp | SigLIP-L에서 -35pp 회귀했던 것 회복 |
| 실내조명 | 36.4% | 60.3% | +23.9pp | 회귀 회복 |
| 에어컨 | 26.7% | 42.9% | +16.2pp | 면적 sub |
| 상 | 55.0% | 61.4% | +6.4pp | |

큰 회귀 — 진열장이 압도적 손실:
| 클래스 | SigLIP-L sub | SigLIP2-L-512 sub | Δ | 추가 정보 |
|---|---|---|---|---|
| **진열장** | 75.0% | **34.8%** | **-40.2pp** | main도 -22.6pp (이중 손실, 단일 클래스 -25건) |
| TV장식장 | 66.0% | 51.1% | -14.9pp | main도 -9.3pp |
| 컴퓨터 | 96.3% | 85.7% | -10.6pp | 표본 작음 |
| 소파 | 64.4% | 56.4% | -8.0pp | 5-way sub |

진열장 단독 분해:
- SigLIP-L: main 44/93 × sub 75% → ~33 sub correct
- SigLIP2-L-512: main 23/93 × sub 34.8% → ~8 sub correct
- **단일 클래스 -25건** (전체 +168건 net 안에서 가장 큰 손실)

→ 추정 원인: 512px 고해상도가 진열장의 도어/선반을 TV stand로 인식 (시각적 sibling). main confusion 진열장→TV장식장 39건, 진열장→책상 20건.

**의사결정:** SigLIP2-L-512 정식 baseline 채택. 진열장 회귀는 알려진 trade-off로 문서화. 회복 lever 후보:
1. **Per-class hybrid 라우팅** — 진열장만 SigLIP-L 사용. 추정 +25건 (+0.6pp E2E)
2. **SigLIP2-SO400M-512** (가용 시) — 더 큰 모델로 진열장 + 사이즈 sub 동시 회복 가능성
3. **사이즈 sub 보조 linear probe** — 잔존 약점 (에어컨 42.9%, 형광등 27.8%, 진열장 34.8%) 직격
4. **의료기 alias 단독 실험** — main 0% 잔존 클래스
...
### ✅ Step 3a~3b (완료): SigLIP2-L-512 + Pipeline v2 E2E
- Stage A: **89.2%** (+9.7pp vs CLIP-L)
- Pipeline v2 E2E: **68.9%** (+19.5pp vs CLIP-L baseline 49.4%)
- 상세: § 2.8 / § 2.9

### 🎯 Step 3c (선택, ~50분): SigLIP2-SO400M-512 시도
가용 시 +1~3pp 추가 가능. 진열장 회귀 + 사이즈 sub 동시 회복 가능성. 메모리 ~3~4GB 필요.
